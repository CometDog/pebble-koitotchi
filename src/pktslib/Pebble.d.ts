...
/**
 * The configuration webview has been closed.
 * The response property (i.e: event.response) contains a string that was passed
 * to Pebble.close() in the webview.
 * If the user exited the webview without saving, the response will be 'CANCELLED'.
 */
type WebViewClosedEvent = Event & {
  type: "webviewclosed";
  response: string;
};
